...
int lagev2d(int evflag, int dmflag, int dof, double* data, int* mn, double* ef, int* ij, double* uv, double* res, int LL)
{
  ...
  if(evflag & EVFLAG_VL) {
	 ...
	 lagcoef_old(EVFLAG_VL, u, us);
	 lagcoef_old(EVFLAG_VL, v, vs);
	 for(int d=0; d<dof; d++)		res[d] = 0;
	 for(int a=0; a<4; a++)
		for(int b=0; b<4; b++) {
		  double coef = us[a]*vs[b]; 
  ...
  if(evflag & EVFLAG_FD) {
	 //exit(-1);
	 scl = double(m)/e;
	 //iC( lagcoef(LL, EVFLAG_FD, u, us) );
	 //iC( lagcoef(LL, EVFLAG_VL, v, vs) );
	 lagcoef_old(EVFLAG_FD, u, us);
	 lagcoef_old(EVFLAG_VL, v, vs);
	 for(int d=0; d<dof; d++)		res[d] = 0;
	 for(int a=0; a<4; a++)
		for(int b=0; b<4; b++) {
	 ...
	 //iC( lagcoef(LL, EVFLAG_FD, v, vs) );
	 lagcoef_old(EVFLAG_VL, u, us);
	 lagcoef_old(EVFLAG_FD, v, vs);
	 for(int d=0; d<dof; d++)		res[d] = 0;
	 for(int a=0; a<4; a++)
		for(int b=0; b<4; b++) {
		  double coef = us[a]*vs[b]; 
  ...
  if(evflag & EVFLAG_SD) {
	 //exit(-1);
	 scl = double(m*m)/(e*e);
	 //iC( lagcoef(LL, EVFLAG_SD, u, us) );
	 //iC( lagcoef(LL, EVFLAG_VL, v, vs) );
	 lagcoef_old(EVFLAG_SD, u, us);
	 lagcoef_old(EVFLAG_VL, v, vs);
	 for(int d=0; d<dof; d++)		res[d] = 0;
	 for(int a=0; a<LL; a++)
		for(int b=0; b<LL; b++) {
	 ...
	 scl = double(n*n)/(f*f);
	 //iC( lagcoef(LL, EVFLAG_VL, u, us) );
	 //iC( lagcoef(LL, EVFLAG_SD, v, vs) );
	 lagcoef_old(EVFLAG_VL, u, us);
	 lagcoef_old(EVFLAG_SD, v, vs);
	 for(int d=0; d<dof; d++)		res[d] = 0;
	 for(int a=0; a<LL; a++)
		for(int b=0; b<LL; b++) {
		  double coef = us[a]*vs[b]; 
...
int lagcoef(int num, int evflag, double u, double* us)
{
  ...
  if(       evflag==EVFLAG_VL) {
	 ...
	 if (n == 4){
		p[0] = -1.0; p[1] = 0.0; p[2] = 1.0; p[3] = 2.0;
		
		P[0][0] = 0.0; P[0][1] = -1.0; P[0][2] = -2.0; P[0][3] = -3.0;
		P[1][0] = 1.0; P[1][1] =  0.0; P[1][2] = -1.0; P[1][3] = -2.0;
		P[2][0] = 2.0; P[2][1] =  1.0; P[2][2] =  0.0; P[2][3] = -1.0;
		P[3][0] = 3.0; P[3][1] =  2.0; P[3][2] =  1.0; P[3][3] =  0.0;

		
		OOP[0][0] =  0.0;  OOP[0][1] = -1.0; OOP[0][2] = -0.5; OOP[0][3] = -0.3333333333333;
		OOP[1][0] =  1.0;  OOP[1][1] =  0.0;  OOP[1][2] = -1.0; OOP[1][3] = -0.5;
		OOP[2][0] =  0.5;  OOP[2][1] =  1.0;  OOP[2][2] =  0.0;  OOP[2][3] = -1.0;
		OOP[3][0] = 0.333333333333333; OOP[3][1] =  0.5;  OOP[3][2] =  1.0;  OOP[3][3] =  0.0;
		
		
	 } else if (n == 8){
		p[0] = -1.0; p[1] = 0.0; p[2] = 1.0; p[3] = 2.0;
		p[4] =  3.0; p[5] = 4.0; p[6] = 5.0; p[7] = 6.0;

		P[0][0] = 0.0; P[0][1] = -1.0; P[0][2] = -2.0; P[0][3] = -3.0; P[0][4] = -4.0; P[0][5] = -5.0; P[0][6] = -6.0; P[0][7] = -7.0;
		P[1][0] = 1.0; P[1][1] =  0.0; P[1][2] = -1.0; P[1][3] = -2.0; P[1][4] = -3.0; P[1][5] = -4.0; P[1][6] = -5.0; P[1][7] = -6.0;
		P[2][0] = 2.0; P[2][1] =  1.0; P[2][2] =  0.0; P[2][3] = -1.0; P[2][4] = -2.0; P[2][5] = -3.0; P[2][6] = -4.0; P[2][7] = -5.0;
		P[3][0] = 3.0; P[3][1] =  2.0; P[3][2] =  1.0; P[3][3] =  0.0; P[3][4] = -1.0; P[3][5] = -2.0; P[3][6] = -3.0; P[3][7] = -4.0;
		P[4][0] = 4.0; P[4][1] =  3.0; P[4][2] =  2.0; P[4][3] =  1.0; P[4][4] =  0.0; P[4][5] = -1.0; P[4][6] = -2.0; P[4][7] = -3.0;
		P[5][0] = 5.0; P[5][1] =  4.0; P[5][2] =  3.0; P[5][3] =  2.0; P[5][4] =  1.0; P[5][5] =  0.0; P[5][6] = -1.0; P[5][7] = -2.0;
		P[6][0] = 6.0; P[6][1] =  5.0; P[6][2] =  4.0; P[6][3] =  3.0; P[6][4] =  2.0; P[6][5] =  1.0; P[6][6] =  0.0; P[6][7] = -1.0;
		P[7][0] = 7.0; P[7][1] =  6.0; P[7][2] =  5.0; P[7][3] =  4.0; P[7][4] =  3.0; P[7][5] =  2.0; P[7][6] =  1.0; P[7][7] =  0.0;

		
		OOP[0][0] =  0.0;  OOP[0][1] = -1.0; OOP[0][2] = -0.5; OOP[0][3] = -0.3333333333333; 
		OOP[0][4] = -0.25; OOP[0][5] = -0.2; OOP[0][6] = -0.1666666666667; OOP[0][7] = -0.142857142857143;

		OOP[1][0] =  1.0;  OOP[1][1] = 0.0; OOP[1][2] = -1.0; OOP[1][3] = -0.5;
		OOP[1][4] = -0.3333333333; OOP[1][5] = -0.25; OOP[1][6] = -0.2; OOP[1][7] = -0.166666666666667;

		OOP[2][0] =  0.5;  OOP[2][1] = 1.0; OOP[2][2] = 0.0; OOP[2][3] = -1.0;
		OOP[2][4] = -0.5; OOP[2][5] = -0.3333333333; OOP[2][6] = -0.25; OOP[2][7] = -0.2;

		OOP[3][0] = 0.3333333333; OOP[3][1] =  0.5;  OOP[3][2] = 1.0; OOP[3][3] = 0.0;
		OOP[3][4] = -1.0; OOP[3][5] = -0.5; OOP[3][6] = -0.3333333333; OOP[3][7] = -0.25;

		OOP[4][0] = 0.25; OOP[4][1] = 0.3333333333; OOP[4][2] =  0.5;  OOP[4][3] = 1.0;
		OOP[4][4] = 0.0; OOP[4][5] = -1.0; OOP[4][6] = -0.5; OOP[4][7] = -0.3333333333;

		OOP[5][0] = 0.2; OOP[5][1] = 0.25; OOP[5][2] = 0.3333333333; OOP[5][3] =  0.5;
		OOP[5][4] = 1.0; OOP[5][5] = 0.0; OOP[5][6] = -1.0; OOP[5][7] = -0.5;

		OOP[6][0] = 0.166666666666667; OOP[6][1] = 0.2; OOP[6][2] = 0.25; OOP[6][3] = 0.3333333333;
		OOP[6][4] =  0.5; OOP[6][5] = 1.0; OOP[6][6] = 0.0; OOP[6][7] = -1.0;

		OOP[7][0] = 0.142857142857143; OOP[7][1] = 0.166666666666667; OOP[7][2] = 0.2; OOP[7][3] = 0.25;
		OOP[7][4] = 0.3333333333; OOP[7][5] =  0.5; OOP[7][6] = 1.0; OOP[7][7] = 0.0;
	 ...
	 */
     ...
     if (n == 4){
        ...
        // 1st derivative of the Lagrange basis polynomials
        //us[j] = L_j'(u) = \sum_{k != j} [1/(x_j - x_k) * 
        //          \prod_{m !=j,k} (u - x_m)/(x_j - x_m)]
       double kth_term, x_j, x_k, x_m;
       for(int j = 0; j < n; j++){ 
         us[j] = 0.;
         x_j = p[j];
         for(int k = 0; k < n; k++){
           if(k != j){
             ...
             for(int m = 0; m < n; m++){
               if(m != k && m != j){
                 x_m = p[m];
                 kth_term *= (u - x_m)/(x_j - x_m);
               }
             }
           // kth_term = [1/(x_j - x_k) * \prod_{m !=j,k} (u - x_m)/(x_j - x_m)]
           us[j] += kth_term;
           }
         }
       }
...
int fftrf2d(int dof, double* data, int* mm, int* rr, double* res)
{
    ...
    // MJM BUG only works for rr[0] = rr[1] = 16
  ebiFunctionBegin;
  //old matrix size: mm[0] = mm[1] = num_samples per patch
  int m1 = mm[0];
  int m2 = mm[1]; 

  //new matrix size: rr[0] = rr[1] = refinement_factor
  // => new matrix size = (num samples per patch)*refinement factor
  int n1 = mm[0]*rr[0];  
  int n2 = mm[1]*rr[1]; 
  ...
  //allocate space
  // Original/refined signal data (i.e. original/refined density)
  double* _sorg = data;//M.data(); //input data
  double* _sref = res; //R.data(); //output data
  
  // Original/refined frequency data (i.e. original/refined density frequency)
  double* _forg = new double[dof * 2*mq1 * m2];
  double* _fref = new double[dof * 2*nq1 * n2];
  
  ebiAssert(_forg!=NULL);
  ebiAssert(_fref!=NULL);
  
  // Zero out _forg and _fref
...
#ifdef FFTW3
  // 2 := 2d fft
  // mm := dimensions of input data
  // dof := # of FFT's to compute
  // _sorg := input data location of size mm[0] x mm[1]
  // NULL :=  rank of input array
  // dof := input FFT stride size
  // 1 := distance in memory from first element of first array to first element
  //        of second array of the input array
  // (fftw_complex*) _forg := fourier transformed output data
  // NULL :=  rank of ouput array
  // dof := ouput FFT stride size
  // 1 := distance in memory from first element of first array to first element
  //        of second array of the ouput array
  ...
  /*
   * Copies the 2D density data on (e.g. on a patch) in the following manner:
   * |-------|              |-------|-------|
   * | 1 | 2 |              | 1 |   |   | 2 |
   * |-------|              |---|---|---|---|
   * | 3 | 4 |              |   |   |   |   |
   ...
   *                        | 3 |   |   | 4 |
   *                        |---|---|---|---|
   * before computing the inverse fourier transform
   */
  //    j direction
  //  Note: mq1 = num_samples/2 +1; mp2 = num_samples/2 +1
  // for the lower i-frequencies and all degrees of freedom..
  for(int i=0; i<dof*mq1; i++) {

	 for(int j=0; j<mq2; j++) {
         // Copy the first (num_samples/2)+1 frequencies to the first 
         // (num_samples/2)+1 slots of the output array
#ifdef FFTW3
      ...
      // m2 = num samples => j=0,..., num_samples/2-1
	 for(int j=0; j<m2-mq2; j++) {
         // Copy the last (num_samples/2)-1 frequencies to the last 
         // (num_samples/2)-1 slots of the output array
#ifdef FFTW3
		//frefmat(i,j+n2-(m2-mq2))[0] = forgmat(i,j+mq2)[0];
		//frefmat(i,j+n2-(m2-mq2))[1] = forgmat(i,j+mq2)[1];
...
#endif	
  //scale
  DblNumVec srefvec(dof*n1*n2, false, _sref);  
  iC( dscal(1.0/double(m1*m2), srefvec) );
  //deallocate space
  delete [] _forg;
  delete [] _fref;
#ifdef FFTW3
...
int csapntinterp(const int srcdof, const DblNumMat& SPOS, const DblNumVec &SDEN, const DblNumMat& TPOS, const Index3 knots, const double ssmth, DblNumVec& TDEN){
  ...
  for (j = 0; j < srcdof; j++){
	 ...
	 double *output;
     // REMOVED FUNCTIONALITY FROM HEDGEHOG
...
int dspntinterp(const int srcdof, const DblNumMat& SPOS, const DblNumVec &SDEN, const DblNumMat& TPOS, DblNumVec& TDEN){
  ...
  for (j = 0; j < srcdof; j++){
	 ...
	 for (k = 0; k < TRGNUM; k++){
         // REMOVED FRUNCITONALITY FROM HEDGEHOG
...
int optimBlendFunc1D(int flags, int deg, double z, double scale,double* res){
  ...
  case 5:
	 ...
	 break;
  case 6:
     z6=z5*z;   z7= z6*z;    z8 = z7*z;  z9 = z8*z;  z10 = z9*z;
	 if      (z<= 0) res[0] = 0.0;
