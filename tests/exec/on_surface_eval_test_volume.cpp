...
void init_volume_density(Vec target_point, 
        Vec sample_point_3d_position, Vec sample_point_normal,
        Kernel3d kernel, Vec volume_density){

    // Compute \phi(y) = (L(y) \cdot n(y))/ D(x,y) for fixed x =target_point, y
    // \in \partial\Omega and L(y) = (y_3, y_1, y_2)

    int num_sample_points= num_local_points(sample_point_3d_position);
    DblNumMat target_point_local = get_local_vector(DIM, 1, target_point);

    DblNumMat sample_point_3d_position_local = get_local_vector(DIM, num_sample_points, sample_point_3d_position);
    DblNumMat sample_point_normal_local = get_local_vector(DIM, num_sample_points, sample_point_normal);
    DblNumMat volume_density_local= get_local_vector(kernel.get_sdof(), num_sample_points, volume_density);
   
    DblNumMat double_layer_potential(1*kernel.get_tdof(), num_sample_points*kernel.get_sdof());
    kernel.kernel(  sample_point_3d_position_local, sample_point_normal_local, target_point_local, double_layer_potential);
    for(int i = 0; i < num_sample_points; i++){
        double y1 = sample_point_3d_position_local(0,i);
        double y2 = sample_point_3d_position_local(1,i);
        double y3 = sample_point_3d_position_local(2,i);
        double n1 = sample_point_normal_local(0,i);
        double n2 = sample_point_normal_local(1,i);
        double n3 = sample_point_normal_local(2,i);

        double L_dot_n = y1*n1;// + y1*n2 + y2*n3;
        for(int d = 0; d < kernel.get_sdof(); d++)
            volume_density_local(d,i) = L_dot_n/double_layer_potential(d,i);
    }

    target_point_local.restore_local_vector();
    volume_density_local.restore_local_vector();
    sample_point_3d_position_local.restore_local_vector();
    sample_point_normal_local.restore_local_vector();
...
int main(int argc, char** argv)
{
    ...
    VecCopy(bis->patch_samples()->sample_as_face_point(), on_surface_face_points);
        ...
        Vec target_point;
        VecCreateMPI(comm, 1*DIM, PETSC_DETERMINE, &target_point);
        DblNumMat tp = get_local_vector(DIM, 1, target_point);
        setvalue(tp, 0.);
        tp.restore_local_vector();
        

        init_volume_density(target_point, 
                bis->patch_samples()->sample_point_3d_position(),
                bis->patch_samples()->sample_point_normal(),
                problem_kernel, den_solved);
        ...
        */
        ...
        qt = VAR_U;
         ...
         eval_time = clock(); 
        bis->fareval(target_point, qt, den_solved, target_value);
        eval_time = clock() - eval_time;
        cout << endl << "Far evaluation time: " << ((double)eval_time/CLOCKS_PER_SEC) << endl << endl; 
        DblNumMat tp2 = get_local_vector(problem_kernel.get_tdof(), 1, target_value);
        cout << "volume? " << endl;
        for(int d =0; d < problem_kernel.get_tdof(); d++)
            cout <<  "d" << d << ": " << tp2(d,0) << endl;
        exit(0);
        cout << "in main()" << endl;
